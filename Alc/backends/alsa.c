...
extern void Get_Pcm_Config(struct pcm_config* config);
...
static ALCboolean ALCplaybackAlsa_reset(ALCplaybackAlsa *self)
{
	...
	Get_Pcm_Config(&config);

	device->UpdateSize  		= config.period_size;
	device->NumUpdates	= config.period_count;
	device->Frequency			= config.rate;
